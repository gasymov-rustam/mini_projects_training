...
interface UserProps extends PropsWithChildren {
  data: IUser;
  addUser: (data: IUser, minus: boolean) => void;
  isAdded: any;
}

export const User: FC<UserProps> = ({ data, addUser, isAdded }) => {
  return (
    <li>
      <div>
      ...
      <img
        className='action'
        src={isAdded ? '/assets/minus.svg' : '/assets/plus.svg'}
        alt='Action'
        onClick={() => addUser(data, isAdded)}
      />
    </li>
  );
};
